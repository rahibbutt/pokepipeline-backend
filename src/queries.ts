...
export const insertOrReplacePokemons = (id: number, name: string, sprite_url: string) => {
  db.prepare(`
...
export const insertPokemonType = (id: number, slot: number, type: Type4) => {
  ...
  const typeRow = db.prepare('SELECT id FROM types WHERE name = ?').get(typeName) as { id: number } | undefined;
  let typeId = typeRow?.id;

  if (!typeId) {
    const insert = db.prepare('INSERT INTO types(name) VALUES (?)').run(typeName);
...
export const insertPokemonAbilities = (id: number, abilities: Ability[]) => {
  ...
  for (const abilityItem of abilities) {
    ...
    const abilityRow = db.prepare('SELECT id FROM abilities WHERE name = ?').get(abilityName) as { id: number } | undefined;
    let abilityId = abilityRow?.id;

    if (!abilityId) {
      const insert = db.prepare('INSERT INTO abilities(name) VALUES (?)').run(abilityName);
...
export const insertPokemonStats = (id: number, stats: Stat[]) => {
  ...
  for (const { base_stat, stat } of stats) {
    ...
    const statRow = db.prepare('SELECT id FROM stats WHERE name = ?').get(statName) as { id: number } | undefined;
    let statId = statRow?.id;

    if (!statId) {
      const insert = db.prepare('INSERT INTO stats(name) VALUES (?)').run(statName);
